...
public function eachincluir($ptl,$eti,$datos='',$sub=null,$start=0,$len=500){ $un = "";
...
public function fetch($ptl,$eti,$arreglo=''){ $us = ""; foreach($arreglo as $key => $vlu){ $row = file_get_contents($ptl.EXT); $row = str_replace("[key]",$vlu, $row); $us .= $row; $this->elem[$eti] = $us; } return $this;}
  ...
  /*********JS_LOADER**********************************************************************/ 
  public function js($salida,$eti){$ext='.js'; if(!is_array($salida)){ $salida = trim(file_get_contents($salida.$ext)); $this->elem[$eti] = '<script>'.$salida.'</script>';  return $this;}else{$css='';foreach ($salida as $salida) { $conv = trim(file_get_contents($salida.$ext)); $css .= "\n".$conv; } $this->elem[$eti] = '<script>'.$css.'</script>';  return $this;} }
  /****************************************************************************************/
  ...
  list($dust,$variable,$content) = $mtodo; foreach($variable as $key => $value) { if(isset($this->elem[$value])&&preg_match('~</each:'.$value.'/>([^}]+?)(?=</endeach:/>|</each:)~',$this->salida,$meach,PREG_OFFSET_CAPTURE)){ list($polvo,$tit) = $meach; $llave=$value; $this->salida = str_replace($tit, "[[[".$llave."]]]", $this->salida); $unidos = ""; foreach($this->elem[$value] as $datos){ $row = $tit[0]; foreach ($datos as $key => $robot) { $row = str_replace('</'.$key.'/>',$robot, $row); } $unidos .=$row; } $this->salida = str_replace("[[[".$llave."]]]",$unidos,$this->salida); $this->salida = str_replace('</each:'.$llave.'/>', " ",$this->salida); }else{ /*si no hay resultados*/ preg_match('~</each:'.$value.'/>([^}]+?)(?=</endeach:/>|</each:)~',$this->salida,$meach,PREG_OFFSET_CAPTURE); list($polvo,$tit) = $meach;$this->salida = str_replace($tit, "", $this->salida); /*si no hay resultados*/ } } $this->salida = str_replace('</endeach:/>',"", $this->salida); return $this;}
  /*********IF***********************AUTO**COMPILADOR***********************************/
  private function si(){

    $patente='~</:si:\(([^}]+?)\)\(([^}]+?)\)\(([^}]+?)\)/>([^}]+?)</:endsi:/>~i';

    while(preg_match_all($patente,$this->salida,$sir)){
    
    if(preg_match($patente,$this->salida,$cumple)){ 

     list($v9,$dvar,$condit,$valores,$contenit) = $cumple;

    /*[variable] [condicion] [valor] [contenido]*/
//IF FILEGETCONTENT CONTENIDO REMOVE ENDSI PUT CONTENT IN IF START TAG MAKE IT GREEDY

  switch (trim($condit)){ case 'igual': if($dvar == $valores){ $resp=true; }else{ $resp =false; } break; case '!igual': if($dvar !== $valores){ $resp =true; }else{ $resp =false; } break; case 'existe': if(isset($dvar)){ $resp =true; }else{ $resp =false; } break; case 'dobleexiste': if(isset($dvar)&&isset($valores)){ $resp =true; }else{ $resp =false; } break; case 'existe!existe': if(isset($dvar)&&!isset($valores)){ $resp =true; }else{ $resp =false; } break;
    case 'lleno': if(!empty($dvar)){ $resp =true; }else{ $resp =false; } break;
     case '!existe': if(!isset($dvar)){ $resp =true; }else{ $resp =false; } break; case 'doble!existe': if(!isset($dvar)&&!isset($valores)){ $resp =true; }else{ $resp =false; } break; case 'menor': if($dvar<$valores){ $resp=true; }else{ $resp=false; } break; case 'mayor': if($dvar>$valores){ $resp =true; }else{ $resp =false; } break; case 'menoroigual': if($dvar<=$valores){ $resp=true; }else{ $resp=false; } break; case 'mayoroigual': if($dvar>=$valores){ $resp =true; }else{ $resp =false; } break; default: echo ' la condicion no existe =>'.$dvar.'-'.$condit.'-'.$valores;$resp =false; break; }
  /*******FIN CONDICION**************/ 
  if($resp===true){ /*****remover etiquetas*****/

   $this->salida = str_replace('</:si:('.$dvar.')('.$condit.')('.$valores.')/>', " ",$this->salida);
    
     }else{
      $this->salida = str_replace('</:si:('.$dvar.')('.$condit.')('.$valores.')/>'.$contenit.'</:endsi:/>',"", $this->salida);
    } } } /*endfor*/ $this->salida = str_replace('</:endsi:/>',"", $this->salida);  return $this;
  }

public function menu(){ $menu='';  foreach (MENU as $key => $value){  

      if('/'.$value['pagina']===$_SERVER['REQUEST_URI']){$activo='id="activo"';}else{$activo='';}
     $menu.=" <a href='/".$value['pagina']."' title='".$value['titulo']."' itemprop='url'".$activo.">".$value['nombre']."</a>";
...
/********POR DEFECTO********************************************************************************/
$this->css(array('css/'.TEMA.'/'.DEFAULTCSS,'css/'.TEMA.'/'.$this->vista.'/'.$this->metodo,'css/font-awesome/css/font-awesome'),'csscontrol')->eti('ti',$this->vista)->js(array('js/jq','dom/'.$this->vista.'/'.$this->metodo,'js/'.JS),'DOMJS')->eti('MENU',$this->menu())->eachset('pienavegacion',MENU);
/**********MAQUINA DE CONSIDENCIAS*******************/ 
while(preg_match_all('~</(\w+?)/>~',$this->salida)){$this->incluyendo(); $this->each(); $this->tags();$this->si(); } 
